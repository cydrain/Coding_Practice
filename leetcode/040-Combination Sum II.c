...
public class Solution {
    public void combinationSum2Helper(int[] num, int target, int index,
                                      List<Integer> p, List<List<Integer>> r) {
        if (target == 0) {
            ...
            for (int i = index; i < num.length; i++) {
                ...
                p.add(num[i]);
                combinationSum2Helper(num, target-num[i], i+1, p, r);
                p.remove(p.size()-1);
            }
        }
    }
    public List<List<Integer>> combinationSum2(int[] num, int target) {
        ...
        Arrays.sort(num);
        combinationSum2Helper(num, target, 0, p, r);
        return r;
    }
}
