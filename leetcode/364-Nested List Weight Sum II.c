...
class Solution {
public:
    void getDepth(vector<NestedInteger>& nestedList, int level, int& maxLevel) {
        maxLevel = max(maxLevel, level);
        for (int i = 0; i < nestedList.size(); i++) {
            if (!nestedList[i].isInteger()) {
                getDepth(nestedList[i].getList(), level+1, maxLevel);
            }
        }
    }
    int depthSumInverse(vector<NestedInteger>& nestedList, int level) {
        ...
        for (i = 0; i < nestedList.size(); i++) {
            if (nestedList[i].isInteger()) {
                ...
                ans += depthSumInverse(nestedList[i].getList(), level-1);
            }
        }
        return ans;
    }
    int depthSumInverse(vector<NestedInteger>& nestedList) {
        int maxLevel = 0;
        getDepth(nestedList, 1, maxLevel);
