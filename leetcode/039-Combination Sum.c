...
public class Solution {
    public void combinationSumHelper(int[] C, int T, int index, List<Integer> p, List<List<Integer>> r) {
        if (T == 0) {
            ...
            for (int i = index; i < C.length; i++) {
                if (C[i] > T) break;
                p.add(C[i]);
                combinationSumHelper(C, T-C[i], i, p, r);
                p.remove(p.size()-1);
            }
        }
    }
    public List<List<Integer>> combinationSum(int[] C, int T) {
        ...
        Arrays.sort(C);
        combinationSumHelper(C, T, 0, p, r);
        return r;
    }
}
