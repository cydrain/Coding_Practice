...
public:
    int lengthLongestPath(string input) {
        ...
        for (i = 0; i < len; i++) {            
            ...
            if (isFile) {
                maxlen = max(maxlen, cnt[l-1]+count);
            } else {
                if (l < cnt.size()) {
                    cnt[l] = cnt[l-1] + count + 1;
                } else {
                    cnt.push_back(cnt.back() + count + 1);
                }
            }
            // reset
            count = 0, l = 1, isFile = false;
        }
        return maxlen;
    }
};
    ...
    int lengthLongestPath(string input) {
        ...
        while (start < len) {
            ...
            if (currdir.find('.') != string::npos) {
                for (i = 0, currlen = 0; i <= level; i++) {
                    currlen += dir[i].length();
                }
                currlen += level;  // add the length for '/'
